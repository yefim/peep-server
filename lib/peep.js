...
var _getOrCreateContact = function(contact) {
  return db.users.findByNumber(contact.number).then(function(s) {
    if (s) {
      ...
      // TODO: refactor this to just use one insert of an array of docs that need creating instead of this crazy loop
      return db.users.insert(s).then(_.first);
    }
  });
};
...
var register = function(user) {
  return _saveUser(user).then(function(result) {
    ...
    return Promise.all(contacts).then(function(contactsToCreate) {
      // db.users.insert(contactsToCreate)
      return result;
    });
  });
};
...
var isRegistered = function(user) {
  return db.users.findByTokenAndNumber(user.token, user.number).then(function(result) {
    return result && result.user.status === 2;
  });
};
